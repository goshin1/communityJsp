...
<% 
	...
	try{
		...
		job = multi.getParameter("sJob");
		deg = multi.getParameter("sDeg");
		exp = multi.getParameter("sExp");
		bArea = multi.getParameter("sBArea");
		sArea = multi.getParameter("sSArea");
		...
		int res = pMgr.modifyJobBoard(num, job, deg, exp, bArea, sArea, company, user, title, content, dbFiles, totalSize);
		if(res <= 0){
			out.println("<script>alert('게시글 수정에 실패하였습니다.'); history.back();</script>");
		}else{
			response.sendRedirect("jobPost.jsp?num="+num);	
		}
